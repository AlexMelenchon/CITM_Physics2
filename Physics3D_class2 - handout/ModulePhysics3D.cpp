...
ModulePhysics3D::ModulePhysics3D(Application* app, bool start_enabled)
	: Module(app, start_enabled), debug_draw(NULL) , dispatcher (NULL), pairCache (NULL), constraintSolver(NULL), collisionConfiguration(NULL)
{
	debug = true;
...
bool ModulePhysics3D::Start()
{
...

	{
		...
		btRigidBody* rectangle;		rectangle = new btRigidBody(0.0f, motionState, shape);
...
update_status ModulePhysics3D::Update(float dt)
{
	...
	if(debug == true)
	{
		...
		if(App->input->GetKey(SDL_SCANCODE_1) == KEY_DOWN)
		{
			...
			matrix.look(-App->camera->Z, App->camera->X, App->camera->Y);
			...
			btRigidBody* sphere;			sphere = new btRigidBody(20.0f, motionState, shape);
