...
bool PhysBody::Contains(int x, int y) const
{
	...
	uint shapeCount = shape->GetChildCount();


	for (uint i = 0; i < shapeCount; i++)
	{
		if (shape[i].TestPoint(this->body->GetTransform() , position))
			return true;
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	b2Vec2 position;
	position.x = PIXEL_TO_METERS(x2);
	position.y = PIXEL_TO_METERS(y2);

	b2Vec2 vector;
	vector.x = x2 - x1;
	vector.y = y2 - y1;

	//Now we calculate the unit vector

	//Then we check it collides, and multiply the normal for the distance that is normal_x and normal_Y down (those are to delete)
