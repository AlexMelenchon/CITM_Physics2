...
bool ModuleSceneIntro::Start()
{
	...
	for (uint n = 0; n < SnakeLength; n++)
	{
		Sphere* s = new Sphere(Size);
		primitives.PushBack(s);
		s->SetPos(XPos, 10.f, 2.5f);

		Primitive* hola = *primitives.At(n);

		//TODO 2: Link all the spheres with your P2P constraint
		if (n > 0)
		{
			Primitive* n1 = *primitives.At(n);
			Primitive* n2 = *primitives.At(n-1);

			App->physics->AddConstraintP2P(*n1, *n2, { 0,0,0 }, {0,0,0});
